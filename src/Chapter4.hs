...
concat' Empty = Empty
concat' (Cons x xs) = extend' x (concat' xs)

extend' :: List a -> List a -> List a
extend' Empty x = x
extend' x Empty = x
extend' (Cons a as) b = Cons a (extend' as b)
...
instance Applicative List where
    ...
    Empty <*> _ = Empty
    Cons f fs <*> x = concat' $ Cons (fmap f x) (Cons (fs <*> x) Empty)
...
-- practice task to better understand binding
notM :: (Monad m) => m Bool -> m Bool
notM x = x >>= (\a -> pure (not a))

andM :: (Monad m) => m Bool -> m Bool -> m Bool
andM a b = a >>= (\x -> if x then b >>= (\y -> pure $ x && y) else pure False)
...
reverse' (Node lt a rt) = Node (reverse' rt) a (reverse' lt)

toList :: Tree a -> [a]
toList (Leaf a) = [a]
toList (Node lt a rt) = toList lt ++ [a] ++ toList rt
